<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML><HEAD><TITLE>Indonesian Liveaboard Katharina Exploration Cruises Boat Details</TITLE>
<META http-equiv=Content-Type content="text/html; charset=iso-8859-1">
<meta name="keywords" content="katharina indonesian liveaboard exploration cruises,Liveaboard Tauchkreuzfahrten in Indonesien,Tauchfahrten,Tauchtrips,Segeltouren,Tauchen,Tauchausbildung,tauchen in Sulawesi,tauchen in Komodo,tauchen in Irian Jaya,Steilwandtauchen,Rifftauchen,Komodo Marine Park,Komodo Drachen,Tauchschiffe in Indonesien,tauchen und schnorcheln in Bali Indonesien,Bali Tauch Hotel Resort,exklusives Liveaboard in Indonesien,">
<STYLE type=text/css></STYLE>
<META content="MSHTML 6.00.2800.1476" name=GENERATOR>
<script language="JavaScript" type="text/JavaScript">
<!--




function MM_jumpMenu(targ,selObj,restore){ //v3.0
  eval(targ+".location='"+selObj.options[selObj.selectedIndex].value+"'");
  if (restore) selObj.selectedIndex=0;
}

function MM_preloadimages() { //v3.0
  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
    var i,j=d.MM_p.length,a=MM_preloadimages.arguments; for(i=0; i<a.length; i++)
    if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
}
//-->
</script>
<style>
  body, td, p, input, select { font-family: Verdana, Arial, Helvetica; font-size: 8pt; color: #000000; }
  a { color: #000000; }
  A:active {
	COLOR: #FF9900;
	TEXT-DECORATION: none
}
A:visited {
	COLOR: #0033FF;
	TEXT-DECORATION: none
}
A:link {
	COLOR: #FF9900;
	TEXT-DECORATION: none
}
A:hover {
	COLOR: #0033FF;
	TEXT-DECORATION: none
}
.style1 {color: #FFFFFF}
.style4 {
	font-size: 12pt;
	font-weight: bold;
	color: #003399;
}
.style6 {
	font-size: 14px;
	font-weight: bold;
}
.style27 {color: #0033CC}
.style32 {	font-size: 10pt;
	font-weight: bold;
}
.style30 {color: #0000FF}
.style33 {font-size: 12px}
.style36 {font-size: 9px}
.style38 {font-size: 9}
.style39 {color: #0033CC; font-size: 9; font-weight: bold; }
.style48 {font-family: Verdana, Arial, Helvetica, sans-serif}
.style49 {font-family: Verdana, Arial, Helvetica, sans-serif;
	font-size: 9px;
	font-weight: bold;
}
.style80 {	color: #0033CC;
	font-weight: bold;
	font-size: 10px;
	font-family: Verdana, Arial, Helvetica, sans-serif;
}
.style82 {font-size: 10px}
.style51 {font-size: 9px;
	font-weight: bold;
}
.style88 {font-size: 12px;
	font-weight: bold;
}
.style88 {font-size: 10px; font-family: Verdana, Arial, Helvetica, sans-serif; font-weight: bold; }
.style56 {
	font-size: 8pt;
	color: #F0F0F0;
}
.style83 {font-size: 12px;
	font-weight: bold;
}
.style83 {font-size: 10px; font-family: Verdana, Arial, Helvetica, sans-serif; font-weight: bold; }
.style89 {color: #009900;
	font-weight: bold;
}
</style>
<script src="data/Scripts/AC_RunActiveContent.js" type="text/javascript"></script>
</HEAD>
<BODY bgColor=#006666 leftMargin=0 topMargin=0 rightMargin=0 bottomMargin=0 marginwidth="0"
marginheight="0" onLoad="MM_preloadimages('mvanggun/deatils2.gif')">
<TABLE width="775" border=0 align="center" cellPadding=0 cellSpacing=0>
  <TBODY>
  <TR>
    <TD>
      <TABLE height=194 cellSpacing=0 cellPadding=0 width="100%"
      background="../images/z3.gif" border=0>
        <TBODY>
        <TR>
              <TD height="194" vAlign=top>
                <TABLE cellSpacing=0 cellPadding=0 width="100%"
            background="../images/dotline.gif"
              border=0><TBODY>
              <TR>
                <TD height=1><IMG height=1
                  src="../images/dotline.gif"
              width=3></TD></TR></TBODY></TABLE>
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td width="332" height="171" valign="bottom" background="../images/z1.jpg">
<TABLE width="180" height=40
                              border=0 align="right" cellPadding=2 cellSpacing=0>
                        <TBODY>
                          <TR>
                            <TD bgcolor="#ffffff"><IMG
                                src="../images/m3.jpg" alt="indonesia_liveaboard_scuba_diving"
                                width=67 height=40></TD>
                            <TD bgcolor="#ffffff"><IMG
                                src="../images/m4.jpg" alt="indonesia_liveaboard_scuba_diving"
                                width=67 height=40></TD>
                            <TD bgcolor="#ffffff"><IMG
                                src="../images/m2.jpg" alt="indonesia_liveaboard_scuba_diving"
                                width=67 height=40></TD>
                          </TR>
                          <TR>
                            <TD colspan="3"><img src="../images/spacer.gif" width="10" height="67"></TD>
                          </TR>
                        </TBODY>
                      </TABLE>
                      <div align="right"></div></td>
                    <td width="123" background="../images/z2.jpg">&nbsp;</td>
                    <td width="320" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td><img src="../images/spacer.gif" width="10" height="45"></td>
                        </tr>
                        <tr>
                          <td height="22" valign="top">&nbsp;&nbsp;<span class="style6"><font color="#FFFFFF" face="Verdana, Arial, Helvetica, sans-serif">&nbsp;<font color="#FFFFFF" face="Verdana, Arial, Helvetica, sans-serif"><a href="../index.php">Home</a> | <a href="contactus.php">Kontaktieren Sie uns</a></font></font>                            </span></td>
                        </tr>
                        <tr>
                          <td><img src="../images/m1.jpg" alt="indonesia_liveaboard_scuba_diving" width="325" height="104"></td>
                        </tr>
                      </table></td>
                  </tr>
                  <tr bgcolor="#006699">
                    <td colspan="3"> <div align="right"><strong><font size="2" face="Arial, Helvetica, sans-serif">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font></strong></div></td>
                  </tr>
                </table></TD></TR></TBODY></TABLE></TD></TR>
  <TR>
    <TD bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=0 width="775" border=0>
        <TBODY>
        <TR>
              <td width=291 align=middle vAlign=top bgcolor="#4DCEE3"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td><table width="278" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                          <td height="176" valign="top" background="../images/bgsearch.jpg">

                            <table width="88%" border="0" align="right" cellpadding="0" cellspacing="0">
              <tr>
                <td height="42">&nbsp;</td>
              </tr>
              <tr>
                <td><?php define( '_VALID_MOS', 1 );  include "../mod/mod_search.php"; ?></td>
              </tr>
            </table></td>
                        </tr>
                      </table></td>
                  </tr>
                  <tr>
                    <td><table width="278" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                          <td height="176" valign="top" background="../images/bgdestination.jpg"><table width="87%" border="0" align="center" cellpadding="0" cellspacing="0">
                            <tr>
                              <td height="33" colspan="2">&nbsp;</td>
                            </tr>
                            <tr>
                              <td width="9%"><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td width="91%" height="18" class="style83"><div align="center"><span class="style27"><strong><span class="style27 style36"><span class="style27 style38"><span class="style1 style27 style49 style51 style48"><span class="style51 style82"><a href="../german/boats/liburan_index.php" class="style83">Bunaken Marine Park - Sulawesi </a></span></span></span></span></strong></span></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><span class="style80"><a href="../german/boats/kararu_index.php">Komodo National Park,Flores,Alor</a></span></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><span class="style27"><strong><span class="style27 style36"><span class="style27 style38"><span class="style1 style27 style49 style51 style48"><span class="style51 style82"><a href="../german/boats/wallacea_index.php" class="style83">Sulwesi- Togian und Banggai Islands </a></span></span></span></span></strong></span></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><a href="../german/boats/moana_index.php" class="style39">Komodo Island &amp; Irian Jaya</a></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><a href="../german/boats/ombak_index.php">Komodo Marine Park </a></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center" class="style83">
                                  <div align="center"><a href="../german/boats/seasafari_index.php" class="style83">Irian, Raja Ampat, Misool, Ambon<br>
                                    &quot;Exclusive Liveaboard&quot; </a></div>
                              </div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><a href="../german/boats/katharina_index.php" class="style83">Bali-Flores-Bali Exploration Cruises&quot;</a></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><a href="../german/boats/jingojango_index.php" class="style83">Flores-Komodo-Bali-Lombok <br>
                                Tauch- &amp; Erkundungsfahrten</a></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><a href="../german/boats/duyung_index.php" class="style83">Selayar &amp; Takabonerate Tauchfahrten</a></div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center" class="style83">
                                  <div align="center"><a href="../german/boats/amira_index.php">Komodo, Alor, Raja Ampat &amp; Papua</a><a href="../german/boats/philkade_index.php"></a></div>
                              </div></td>
                            </tr>
                            <tr>
                              <td>&nbsp;</td>
                              <td height="18" class="style1 style48 style82"><div align="center"></div></td>
                            </tr>
                            <tr>
                              <td><div align="center"><strong><img src="images/dot.gif" width="4" height="4"></strong></div></td>
                              <td height="18" class="style83"><div align="center"><a href="../german/boats/ondina_index.php">Maluku, Bira, Selayar, Irian Jaya</a></div></td>
                            </tr>
                            <tr>
                              <td height="18" colspan="2"><div align="center">
                                  <p><br>
                                      <img src="../images/hot-deals.jpg" alt="Sonderangebote &amp; Last Minute" width="103" height="24" border="2" usemap="#Map"></p>
                              </div></td>
                            </tr>
                            <tr>
                              <td colspan="2"><img src="images/spacer.gif" width="10" height="8"></td>
                            </tr>
                          </table>
                            <map name="Map">
                              <area shape="rect" coords="-5,-1,104,27" href="dive_resort_tulamben/index.php" target="_blank" alt="Tauchkreuzfahrt Sonderangebote &amp; Last Minute">
                            </map></td>
                        </tr>
                    </table></td>
                  </tr>
                </table>
                <p>&nbsp;</p>
                <p align="center"><img src="../dive_resort_tulamben/resortimages/banner.jpg" alt="Bali Tauch Hotel Resort mit moderner Tauchbasis &amp; Schiffwrack" width="283" height="85" border="0" usemap="#Map2"><br>
                    <a href="../dive_resort_tulamben/index.php" class="style32">Bali Tauch Hotel Resort mit <br>
                      moderner Tauchbasis &amp; Schiffwrack</a></p>
                <map name="Map2">
                  <area shape="rect" coords="296,41,343,44" href="#">
                  <area shape="rect" coords="0,0,303,88" href="dive_resort_tulamben/index.php" target="_blank" alt="Bali Tauch Hotel Resort mit moderner Tauchbasis &amp; Schiffwrack">
                </map>
                <p align="center">                  <br>
                </p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center">&nbsp;</p>
                <p align="center"><img src="katharina/katharina_cruises.jpg" alt="katharina liveaboard diving" width="250" height="188"> </p>
              <p align="center">&nbsp;                    </p></td>
          <TD vAlign=top width=10
          background="../images/z6.jpg"><IMG height=4
            src="../images/z6.jpg" width=5></TD>
          <TD width="474" vAlign=top><table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
              <td><div align="center"></div></td>
            </tr>
            <tr>
              <td><table width="100%"  border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td bgcolor="#2E3C9D"><table width="100%"  border="0" cellpadding="1" cellspacing="1">
                    <tr>
                      <td height="18" bgcolor="#006699" class="style1">&nbsp;<strong>Katharina Bootsspezifikationen</strong></td>
                    </tr>
                    <tr>
                      <td bgcolor="#FFFFFF"><table width="100%"  border="0" cellpadding="2" cellspacing="2">
                          <tr>
                            <td width="51%"><div align="center"><img src="katharina/katharina_boat.jpg" alt="katharina sailing boat" width="210" height="154"></div></td>
                            <td width="49%"><br>
                              <br>
                              <table width="189" border="0" cellpadding="0" cellspacing="0">
                              <tr>
                                <td width="189" bgcolor="#006699"><table width="100%"  border="0" cellpadding="1" cellspacing="1">
                                  <tr>
                                    <td height="18" bgcolor="#D9F2FF"><div align="left"><strong><a href="katharina_index.php">&Uuml;ber  Katharina Kreuzfahrten </a></strong></div></td>
                                  </tr>
                                  <tr>
                                    <td height="18" bgcolor="#D9F2FF"><a href="katharina_boatdetails.php"><strong>Katharina Bootsspezifikationen</strong></a></td>
                                  </tr>
                                  <tr>
                                    <td height="18" bgcolor="#D9F2FF"><div align="left"><strong><a href="katharina_scheduleandrates.php">Programm &amp; Preise</a> </strong></div></td>
                                  </tr>
                                  <tr>
                                    <td height="18" bgcolor="#D9F2FF"><div align="left"><strong><a href="katharina_divedestinations&itenary.php">Tauchziele</a></strong></div></td>
                                  </tr>
                                </table></td>
                                  </tr>
                              </table>
                              <blockquote>&nbsp;</blockquote></td></tr>
                          <tr>
                            <td colspan="2"><br>
                              <p align="center"><span class="style4"><strong>Katharina</strong> Bootsspezifikationen</span></p>
                                <table border="0" cellspacing="0" cellpadding="0" width="97%">
                                  <tr>
                                    <td width="29%" valign="top"><p><strong>Name</strong></p></td>
                                    <td width="2%" valign="top"><p align="center"><strong>:</strong></p></td>
                                    <td width="69%" valign="top"><p><strong>Katharina</strong></p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top">Kabinen</td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>7    klimatisierte Kabinen mit privater Dusche und Toilette.<br>
                                      1    x Doppelkabine - (1 Doppelbett, Oberdeck)<br>
                                      4    x Dreierkabine - (1 Doppelbett + 1 Koje, Unterdeck) 2    x Zwillingskabine - (2 Kojen, Oberdeck)</p>
                                      </td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Flag</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>Indonesien</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Gesamtl&auml;nge</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>33    m</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Deck L&auml;nge    </p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>26,5    m</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Tiefgang</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>8,5    m</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Maschine</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>10    Zyl. Mercedes Benz OM 403</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Kreuzgeschwindigkeit</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>8 Knoten</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>H&ouml;chstegeschwindigkeit </p></td>
                                    <td width="2%" valign="top"><p align="center">&nbsp;</p></td>
                                    <td width="69%" valign="top"><p>10 Knoten    </p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Wassertanks</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>14,000    l</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Treibstofftanks</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>9,000    l</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Elektrizit&auml;t</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>49    KVA Generator<br>
                                      MWM/25 KVA Generator<br>
                                      MWM 24 volts Notstromaggregat</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Beiboote</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>2,    ein kleines/ein gro&szlig;es Fiberglasboot</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Navigation</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>Radar    (Reichweite 24 nautische Meilen)<br>
                                      Furuno Echolot </p>
                                      <p>GPS - Weit-/Kurzwelle<br>
                                          Satellitentelefon</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Sicherheit</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>36 Rettungswesten<br>
                                      Leuchtraketen<br>
                                      Notleuchfeuer<br>
                                      2 Rettungsflo&szlig;e (20/25 Personen)</p></td>
                                  </tr>
                                  <tr>
                                    <td width="29%" valign="top"><p>Au&szlig;enbordmotoren</p></td>
                                    <td width="2%" valign="top"><p align="center">:</p></td>
                                    <td width="69%" valign="top"><p>15    &amp; 50 PS Yamaha</p></td>
                                  </tr>
                                </table>
                                <p align="center" class="style33">&nbsp;</p>
                                <table width="97%"  border="0" align="center" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td><div align="center"><img src="katharina/katharina_cabin.jpg" alt="katharina kabine" width="200" height="267"></div></td>
                                    <td><div align="center"><img src="katharina/dbl_cabin_bathroom.jpg" alt="katharina kabine" width="200" height="267"></div></td>
                                  </tr>
                                </table>
                                <p style="margin-top: 0px;">&nbsp;</p>
                                <p align="center"><img src="katharina/katharina_deckplan.jpg" alt="katharina deck plan" width="300" height="579"></p>
                                <div align="center"><br>
                                    <img src="katharina/katarina_cabinplan.jpg" alt="katharina cabin plan" width="460" height="299"></div></td>
                          </tr>
                      </table></td>
                    </tr>
                  </table></td>
                </tr>
              </table></td>
            </tr>
          </table>
            <div align="center">
              <p>                <span class="style32"><a href="#">- Zur&uuml;ck zum Anfang</a></span></p>
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td><span class="style56">Liveaboard Tauchkreuzfahrten in Indonesien,Tauchfahrten,Tauchtrips,<br>
Segeltouren,Tauchen,Tauchausbildung,tauchen in Sulawesi,tauchen in<br>
Alor,tauchen in Komodo,tauchen in Irian Jaya,Steilwandtauchen,Rifftauchen,<br>
Komodo Marine Park,Komodo Drachen,Bunaken Marine Park,Tauchschiffe<br>
in Indonesien,tauchen und schnorcheln in Bali Indonesien,Bali Tauch Hotel<br>
Resort,exklusives Liveaboard in Indonesien</span></td>
                </tr>
              </table>
              </div></TD>
        </TR></TBODY></TABLE></TD></TR>
  <TR>
    <TD>
      <TABLE height=26 cellSpacing=0 cellPadding=0 width="100%"
      background="../images/z4.gif" border=0>
          <TBODY>
            <TR>
              <TD height="8"><img src="../images/spacer.gif" width="10" height="8"></TD>
            </TR>
            <TR>
              <TD height="18" bgcolor="#FFFFFF"><div align="center"><strong><br>
                Jl. Danau Tamblingan X, No. 31 <br>
80000 Jimbaran, Bali, Indonesia<br>
Phone: +62-361 - 774504/772920<br>
Fax: +62-361 -778473 <br>
<a href="mailto:cruises@indonesialiveaboard.com" class="style30">FOR BOOKINGS &amp; RESEVATIONS PLEASE EMAIL US</a><br>
2005 &copy; Copyright IndonesiaLiveaboard. All rights Reserved<br>
              </strong></div></TD>
            </TR>
          </TBODY>
</TABLE></TD></TR></TBODY></TABLE></BODY></HTML>
